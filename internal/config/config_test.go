...
func TestLoadServerConfig(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			env: map[string]string{
				...
				"EMAIL_PERSONAL_EMAIL": "me@example.com",
				"EMAIL_PORT":           "587",
				"EMAIL_TLS":            "true",
			...
			want: ServerConfig{
				...
				EmailPersonalEmail: "me@example.com",
				EmailPort:          "587",
				EmailTLS:           "true",
	...
	keys := []string{"LOG_FORMAT", "LOG_LEVEL", "PORT", "STATE_PATH", "CHECK_INTERVAL", "IP_URLS", "EMAIL_HOST_NAME", "EMAIL_PASSWORD", "EMAIL_PERSONAL_EMAIL", "EMAIL_PORT", "EMAIL_TLS", "EMAIL_USERNAME"}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			for _, k := range keys {
				t.Setenv(k, "")
